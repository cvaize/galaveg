...
1) Singleton глобальные объекты создаются в файле `bootstrap/singleton/singleton.go` и используются в директориях `cmd`,
`app/controllers`, `app/view` для упрощения, ускорения разработки и уменьшения количества кода. Следовательно для
тестирования команд, контроллеров и представлений будут использованы сторонние инструменты.