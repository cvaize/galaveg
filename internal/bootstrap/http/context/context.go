...
func Must(cfg *config.Config) *Context {
	...
	ctx.Services.Users = errors.Must(users.NewService(utils.Must(users.NewDbRepoImpl(users.DbRepoImplSettings{Db: ctx.Infra.Db, Prefix: cfg.Db.Prefix}))))
	...
	ctx.Services.Roles = errors.Must(roles.NewService(utils.Must(roles.NewDbRepoImpl(roles.DbRepoImplSettings{Db: ctx.Infra.Db, Prefix: cfg.Db.Prefix}))))
	...
	ctx.Services.Auth = errors.Must(auth.NewService(ctx.Services.Hash, auth.NewDbRepoImpl(auth.DbRepoImplSettings{Db: ctx.Infra.Db, Prefix: cfg.Db.Prefix})))
	return ctx
}
